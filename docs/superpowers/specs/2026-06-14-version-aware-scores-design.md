...
- Manual features (the 17 path-less features): **as built, the foundation resolves manual features to their current state at all versions** — `loadSupportAtVersion` falls back to `MANUAL_SUPPORT[id]` unchanged, without comparing the requested version against any embedded `*Version` field. Honoring per-browser `*Version` anchors (e.g. `apple-pay` `safari_iosVersion: "10.1"`, `push-api` `safari_iosVersion: "16.4"`) via `compareVersions` is a **deferred refinement** (tracked in `PWAscore-8ii`), not implemented here. This is a small, acknowledged inaccuracy on a handful of features, not score-breaking (their support is binary and mostly stable), and only affects historical points before a manual feature's true introduction.
...
**Experimental status is version-independent.** `shouldExcludeFromPrimaryScore` reads `support.status` (experimental / standard*track / deprecated), which comes from BCD `__compat.status` — not from per-version support. So `experimentalFeatureIds`/`experimentalGroupIds` (precomputed in `onMounted`) do NOT need recomputation on version switch; only support \_levels* change per version.
